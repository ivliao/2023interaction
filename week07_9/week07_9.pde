...
void draw()
{
  ...
  boolean green_part1= false,green_ok=true;
  for (int i=0; i<pt.size()-1; i++){ // -1下面要加回來
    ...
    else if(green_part1==false && green_rightup>150)green_ok =true;
  }
  println(dpx, dpy);
  if(dpx>100 && dpy<40)stroke(255,0,0);//紅
  else if (dpx<40 && dpy>100) stroke(0,0,255);//藍
  else if (green_ok)stroke(0,255,0);//綠
